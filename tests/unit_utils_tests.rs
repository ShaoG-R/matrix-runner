...
fn create_test_dir_structure(base_path: &Path) -> std::io::Result<()> {
    ...
    fs::write(base_path.join("subdir").join("nested").join("file4.txt"), "content4")?;
...
fn verify_dir_structure(base_path: &Path) -> std::io::Result<bool> {
    ...
    let file4_exists = base_path.join("subdir").join("nested").join("file4.txt").exists();
    ...
    Ok(content1 == "content1" && content2 == "content2" && 
...
mod copy_dir_all_tests {
    ...
    fn test_copy_dir_all_preserves_directory_structure() {
        ...
        // Verify file contents
        assert_eq!(fs::read_to_string(dst_dir.join("a").join("file_a.txt")).unwrap(), "a");
        assert_eq!(fs::read_to_string(dst_dir.join("a").join("b").join("file_b.txt")).unwrap(), "b");
        assert_eq!(fs::read_to_string(dst_dir.join("x").join("file_x.txt")).unwrap(), "x");
    }
}
